...
import { Box, ListItemText, Typography, Dialog, DialogTitle, DialogContent, DialogActions, ListItem, List, Divider, ButtonGroup } from "@mui/material";
...
import { ReleasePreview, diffStr, releasePreviewBase, releasePreviewTarget } from "./MockData";
...
const CompareDialog: React.FC<{ open: boolean, setOpen: (open: boolean) => void }> = ({ open, setOpen }) => {
    ...
    <Dialog open={open} onClose={() => setOpen(false)} maxWidth='xl'>
      <DialogTitle>
        <Typography variant="h3" sx={{ p: 1, fontWeight: "bold", color: "mainContent.dark" }}>
          <FormattedMessage id="compare.dialog.title" />
        </Typography>
      </DialogTitle>
...
const CompareView: React.FC = () => {
  ...
  return (
    <Box sx={{ paddingBottom: 1, m: 2 }}>
      ...
      <CompareDialog open={open} setOpen={setOpen} />
    </Box >
  );
}
